...
const bsonDate = (isoDate) =>
{
    ...
    if (!date.isValid)
    {
        throw new Error(`Cannot parse date ${isoDate}: ${date.invalidReason}. ${date.invalidExplanation}.`);
    }
...
const seedMainAsync = async () =>
{
    ...
    console.log("Drop users from database");
    await User.deleteMany({}).exec();

    console.log("Drop places from database");
    await Place.deleteMany({}).exec();

    console.log("Add users to database");
    for (const [_user, data] of Object.entries(users))
    {
        await new User(data).save();
    }

    console.log("Add places to database");
    for (const [_place, data] of Object.entries(places))
    {
        await new Place(data).save();
    }
