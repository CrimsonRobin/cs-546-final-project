...
import {get as getProduct} from "./products.js";
...
const recomputeAverageRating = async (productId) =>
{
    ...
    const aggregation = await allPosts.aggregate([
        {$match: {_id: productId}},
        {$limit: 1},
...
export const updateReview = async (reviewId, updateObject) =>
{
    ...
    const product =
        await collection.findOne({"reviews._id": reviewIdObject});
    if (!product)
    {
...
export const removeReview = async (reviewId) =>
{
    ...
    const productWithReview =
        await collection.findOne({"reviews._id": reviewIdObject});
    const updateResult =
        await collection.updateOne({"reviews._id": reviewIdObject}, {$pull: {"reviews": {_id: reviewIdObject}}});
    if (!updateResult)
    {
