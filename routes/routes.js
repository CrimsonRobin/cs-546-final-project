    ...
    .get(async (req, res) => {
        ...
        if (req.query.latitude === undefined && req.query.longitude === undefined && req.query.radius === undefined && req.query.searchTerm === undefined) {
            ...
            return res.render('searchResults', {
                title: "Search Results", layout: false, message: searchResults,
        ...
        if (errors.length > 0) {
            return res.render('searchResults', {
                title: "Search Results", layout: false, errors: errors,
                user: req.session ? req.session.user : undefined
            });
        }
        ...
        if (req.query.searchTerm) {
            ...
            return res.render('searchResults', {
                title: "Search Results", layout: false, message: `Places matching ${req.query.searchTerm}`, results: searchResults,
                user: req.session ? req.session.user : undefined
            });
        } else {
            searchResults = await findAllNear(req.query.latitude, req.query.longitude, req.query.radius);
            return res.render('searchResults', {
                title: "Search Results", layout: false, message: `Places near you`, results: searchResults,
                user: req.session ? req.session.user : undefined
            });
        }
    });
